...
const defaultConfettiConfig = {
  ...
  confettiRadius: 24,
  confettiColors: [], // Empty array to remove colors
  emojies: [], // Empty array to remove emojis
  svgIcons: [
    "images/hardboiled.png",
    "images/medium_boiled.png",
    "images/over_easy.png",
    "images/over_hard.png",
    "images/soft_boiled.png",
    "images/sunny_egg.png",
  ],
};

// Confetti class representing individual confetti pieces
class Confetti {
  constructor({ initialPosition, direction, radius, svgIcon }) {
    const speedFactor =
      Utils.getRandomInRange(0.9, 1.7, 3) * Utils.getScaleFactor();
    this.speed = { x: speedFactor, y: speedFactor };
    this.finalSpeedX = Utils.getRandomInRange(0.2, 0.6, 3);
    this.rotationSpeed = 0.01; // Default for SVGs
  ...
  draw(context) {
    ...
    // Emojis and colors are no longer drawn
  }
  ...
  updatePosition(deltaTime, currentTime) {
    ...
    // Emoji/SVG confetti don't have the "spin" effect
  }
...
class ConfettiManager {
  ...
  addConfetti(config = {}) {
    ...
    for (let i = 0; i < confettiesNumber / 2; i++) {
      const selectedSvgIcon = Utils.getRandomItem(svgIcons); // Select a random SVG
      this.confetti.push(
        new Confetti({
          initialPosition: { x: 0, y: baseY },
          direction: "right",
          radius: confettiRadius,
          svgIcon: selectedSvgIcon,
        })
      );
      this.confetti.push(
        new Confetti({
